#! /usr/bin/env python
# install of Latex C:\Users\willi\AppData\Local\Programs\MiKTeX
# installabe found at On Windows:
# You can download and install MiKTeX, which is a popular LaTeX distribution for Windows.
# You can download it from the MiKTeX website (https://miktex.org/download) and follow
# the installation instructions
# You must also set path for this installation
import time
...
# pip show scikit-learn  - May have to install a second time after other modules loaded
...
class ColorBooked(object):
    ...
    def visualize_results_3D(self):
        ...
        ax1.scatter(reduced[:, 0], reduced[:, 1], reduced[:, 2],
                    c=[(reducedc[i, 0] / 255, reducedc[i, 1] / 255, reducedc[i, 2] / 255) for i in
                       range(len(reducedc))])
    ...
    def get_copyright():
        ...
        return f'William Collins, Copyright {datetime.now().year}'

    def get_images_and_display():
        # Set the title of the app sidebar
        st.sidebar.subheader('Image Gallery')
        st.sidebar.caption(ColorBooked.get_copyright())

        # Path to your image directory
        image_dir = './images-aig'

        # List all files in the directory
        files = os.listdir(image_dir)

        # Filter out all non-image files and display them in the sidebar
        for file in files:
            if file.endswith(('png', 'jpg', 'jpeg', 'gif')):
                image_path = os.path.join(image_dir, file)
                image = Image.open(image_path)
                st.sidebar.image(image, caption=file, use_column_width=True)

    import time
    def main():
        st.title("Color Book Generator")
        # show image of William
        with open("./author/me.jpg", "rb") as f:
            image_bytes = f.read()
        st.sidebar.image(image_bytes, width=150, caption="William W. Collins", use_column_width=False)

        ColorBooked.get_images_and_display()
        ...
        with st.spinner(f'processing...'):
            ...
            time.sleep(0.5)
            uploaded_file = st.file_uploader("Select a file", type=["jpg", "png"])

            # Define the target directory to save the file
            target_directory = "./images-aig/"

            # Save the uploaded file with a new name (if uploaded)
            if uploaded_file is not None:
                # Create the target directory if it doesn't exist
                # os.makedirs(target_directory, exist_ok=True) # dir exist

                # Generate a new filename (e.g., using a timestamp)
                new_filename = os.path.join(target_directory, uploaded_file.name)  # Modify the filename as needed
                ...
                # now process the file using main.py (convert to lib file later todo)

                with st.expander(f'file information'):
                    file_name = uploaded_file.name
                    st.write('filename:', file_name)
                    full_file_path = os.path.join(os.getcwd(), file_name)
                    st.write(f'full_file_path = {full_file_path}')
                    relative_file_path = os.path.relpath(full_file_path, os.getcwd())
                    st.write(f'relative fp {relative_file_path}')
                    folder_path = os.path.dirname(relative_file_path)
                    st.write(f'folder_path {folder_path}')

                st.write(f"File Path: {file_name}")

                with st.spinner('processing image...'):
                    command = f'python main.py {target_directory}{file_name}'
                    # command = "python main.py ./images-aig/DALLE-20231130_002400_0a.png"
                    st.caption(f'processing command: {command}')
                    time.sleep(3)
                ...
                with st.expander("command output:"):
                    with st.spinner(f'processing image...'):
                        try:
                            ...
                            time.sleep(3)
                            st.caption(output)
                            st.success(f'Completed! Command: {command}')
                        except Exception as e:
                            st.caption (f'Error: {e}')

                try: # show original file and 2 new processed files
                    # file_name is var for original file name
                    append_colorized = '_colorized10.jpg'
                    file_name = file_name.replace('.jpg','')
                    show_file = f'./images_aig/{file_name}{append_colorized}'
                    st.caption(show_file)

                    with open(show_file, "rb") as f:
                        image_bytes = f.read()
                    st.image(image_bytes, caption="Local Image")
                except Exception as e:
                    st.error(f'The file you have tried to process may have encountered challenges.'
                             f' Please look to the panel on the left to check if the file was '
                             f'processed.  There should be 3 files after your original including '
                             f'a palette and colorbook line image.')

                # Original file format (processed):  ./images-aig/DALLE-20231130_002400_0a.png
                #Colorbook outputs 2 files in the following format (append to original file)
                # DALLE-20231130_002400_0a_outlined10_blur2.0_canny0.1.jpg -example of colorbook output
                # DALLE-20231130_002400_0a_colorized10.png
